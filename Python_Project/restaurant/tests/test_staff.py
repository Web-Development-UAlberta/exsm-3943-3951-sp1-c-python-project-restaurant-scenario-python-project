...
@pytest.mark.django_db
def test_assign_server_to_order(client):
    """Test assigning a server to an order"""
    order = Order.objects.create(
        order_type=Order.OrderType.DINE_IN,
        sub_total=30.00,
        total_price=30.00,
    ...
    server = User.objects.create_user(username='srv1', password='pass123', role=User.Role.SERVER_HOST)
    
    client.login(username='srv1', password='pass123')
    response = client.post(
        reverse('assign_server_to_order', args=[order.id]),
        {'server_id': server.id}
    )
...
def test_unauthorized_user_cannot_update_table_status(client):
    ...
    # Should redirect (usually to login or staff index) instead of success
    assert response.status_code == 302
    # Optional: check it doesn't actually update the table
    table.refresh_from_db()
